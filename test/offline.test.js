// this tests real packets/identity from official clients and known-good traffic

import { describe, test } from 'node:test'
import assert from 'node:assert'

// prettier-ignore
import {
  build_announce,
  build_proof,
  bytesToHex,
  get_identity_destination_hash,
  get_message_id,
  hexToBytes,
  lxmf_build,
  lxmf_parse,
  message_decrypt,
  packet_pack,
  packet_unpack,
  private_identity,
  private_ratchet,
  public_identity,
  public_ratchet,
  validate_announce,
  validate_proof,
  PACKET_ANNOUNCE,
  PACKET_DATA,
  PACKET_PROOF
} from '../src/index.js'

const keys = {
  '072ec44973a8dee8e28d230fb4af8fe4': hexToBytes('205131cb9672eaec8a582e8e018307f2428c4aac5e383f12e94939e672b931677763c7398d0b9cb6ef1369d023d8af10b85d80f6579c55a6f528953265c15313'),
  '76a93cda889a8c0a88451e02d53fd8b9': hexToBytes('e8c5c096166f3554868de9133b0c55c7abf0318230860a142ea3f84a0aae7759142f6c0b84d9f537ceb2e8e9678fc9fb77caf91e2176278fb4c4f5c3eb7b48cd')
}

const ratchets = [hexToBytes('205cb256c44d4d3939bdc02e2a9667de4214cbcc651bbdc0a318acf7ec68b066'), hexToBytes('28dd4da561a9bc0cb7d644a4487c01cbe32b01718a21f18905f5611b110a5c45')]

const packets = `
  ANNOUNCE (072ec44973a8dee8e28d230fb4af8fe4):  
  ANNOUNCE (76a93cda889a8c0a88451e02d53fd8b9):  
  DATA (76a93cda889a8c0a88451e02d53fd8b9):  
  PROOF (2831d76f1a8035638505c132fe5818c1):  03002831d76f1a8035638505c132fe5818c100b90b83a04be319463f930b123b667eaaf64a85e827c34831a032cf72834a1dc58836e1fe4c49e30decab52747da2811db83a4b0b8464aa31e02f2eebbf1dae03
  DATA (072ec44973a8dee8e28d230fb4af8fe4):  
  PROOF (d7c0e833f0cbde9f9133cd9e7d508b1a):  0300d7c0e833f0cbde9f9133cd9e7d508b1a00cd00ce237471609d6ef64e427151fed46d9eb71fe6337f6fc530a9f3a55c730f1fd09f82f7d12d1caadbc185b7703f0d9f5db6c792c2dfcdf1eed3111088860c

  `
  .split('\n')
  .filter((l) => l.trim())
  .map((l) => hexToBytes(l.split(':  ').pop()))

// problematic ANNOUNCE packets from other networks
packets.push(hexToBytes(''))
packets.push(hexToBytes(''))

// Track DATA packets for lookup in PROOF
// Normally this would happen when you send a message, but I do it in my DATA tests
const sentPackets = {}

// Build recipients lookup table
const recipients = {}

// Build public keys first (synchronously before tests run)
const pubkeys = {}
for (const hash of Object.keys(keys)) {
  pubkeys[hash] = public_identity(keys[hash])
  recipients[hash] = { priv: keys[hash], pub: pubkeys[hash] }
}

describe('Identity', () => {
  for (const hash of Object.keys(keys)) {
    test(`Client Identity ${hash}`, () => {
      assert.ok(pubkeys[hash])
    })
  }
})

describe('ANNOUNCE', () => {
  test('072ec44973a8dee8e28d230fb4af8fe4', () => {
    const packet = packet_unpack(packets[0])
    assert.equal(packet.packet_type, PACKET_ANNOUNCE)
    assert.equal(bytesToHex(packet.destination_hash), '072ec44973a8dee8e28d230fb4af8fe4')
    assert.equal(bytesToHex(packet.packet_hash), 'e56755f8b7405b07c12a5c25d7b9b744ca296f7349768b335c78be868530b57d')
    const announce = validate_announce(packet)
    assert.ok(announce)
  })
  test('76a93cda889a8c0a88451e02d53fd8b9', () => {
    const packet = packet_unpack(packets[1])
    assert.equal(packet.packet_type, PACKET_ANNOUNCE)
    assert.equal(bytesToHex(packet.destination_hash), '76a93cda889a8c0a88451e02d53fd8b9')
    assert.equal(bytesToHex(packet.packet_hash), '1d17ee4b806f343804567c56ac9d1204e06ef9bc0d1e44b3970f4138a8ef897b')
    const announce = validate_announce(packet)
    assert.ok(announce)
  })

  test('7d62e355cc90ec4e79569d33a8ad6c6b', () => {
    const packet = packet_unpack(packets[6])
    assert.equal(packet.packet_type, PACKET_ANNOUNCE)
    assert.equal(bytesToHex(packet.destination_hash), '7d62e355cc90ec4e79569d33a8ad6c6b')
    assert.equal(bytesToHex(packet.packet_hash), '108b781ce8b8029f8335fc4a4b8a295895c3878d36467bb88da7137c88d3c282')
    const announce = validate_announce(packet)
    assert.ok(announce)
  })

  test('acd4eef4901f2b7c69e761dc8781ed4c', () => {
    const packet = packet_unpack(packets[7])
    assert.equal(packet.packet_type, PACKET_ANNOUNCE)
    assert.equal(bytesToHex(packet.destination_hash), 'acd4eef4901f2b7c69e761dc8781ed4c')
    assert.equal(bytesToHex(packet.packet_hash), 'b04b846ba727d26ea2a0911b37a2c18460d975fe6816f3718f4953b8d1a6ef77')
    const announce = validate_announce(packet)
    assert.ok(announce)
  })
})

describe('LXMF', () => {
  test('2831d76f1a8035638505c132fe5818c1 (A -> B)', () => {
    const packet = packet_unpack(packets[2])
    assert.equal(packet.packet_type, PACKET_DATA)
    assert.equal(bytesToHex(packet.packet_hash), '2831d76f1a8035638505c132fe5818c1d1d25869a973d35c197d669f0d5074d8')

    // it's to Client B
    assert.equal(bytesToHex(packet.destination_hash), '76a93cda889a8c0a88451e02d53fd8b9')
    const recipient = recipients[bytesToHex(packet.destination_hash)]
    assert.ok(recipient)

    // Decrypt the packet data first
    const decrypted = message_decrypt(packet, recipient.pub, ratchets)
    assert.ok(decrypted, 'message should decrypt')

    // Parse the LXMF message
    const message = lxmf_parse(decrypted, packet.destination_hash, pubkeys['072ec44973a8dee8e28d230fb4af8fe4'])
    assert.ok(message)

    assert.equal(bytesToHex(message.source_hash), '072ec44973a8dee8e28d230fb4af8fe4')
    assert.equal(bytesToHex(message.signature), '7b9beae3f07ab3255f0c77fe295ddca70b032fd45735252025eb32dcfe9b278a9f1891ef96d2291a9f8289de000ca695d4586c8d1a846100621f01aa73134a00')
    assert.equal(message.timestamp, 1759383635.953418)

    // Decode title and content if they are Uint8Array
    const title = message.title instanceof Uint8Array ? new TextDecoder().decode(message.title) : message.title
    const content = message.content instanceof Uint8Array ? new TextDecoder().decode(message.content) : message.content

    assert.equal(title, '')
    assert.equal(content, 'hello from A')
    // Convert Map to object for comparison
    const fields = message.fields instanceof Map ? Object.fromEntries(message.fields) : message.fields
    assert.deepEqual(fields, {})

    // Verify signature
    const sender = recipients[bytesToHex(message.source_hash)]
    assert.ok(sender, 'valid sender')
    assert.ok(message.valid, 'valid message signature')

    // save for PROOF validation
    sentPackets[bytesToHex(packet.packet_hash.slice(0, 16))] = {
      recipientHash: packet.destination_hash,
      packet_hash: packet.packet_hash,
      raw: packets[2]
    }
  })

  test('d7c0e833f0cbde9f9133cd9e7d508b1a (B -> A)', () => {
    const packet = packet_unpack(packets[4])
    assert.equal(packet.packet_type, PACKET_DATA)
    assert.equal(bytesToHex(packet.packet_hash), 'd7c0e833f0cbde9f9133cd9e7d508b1a61d2c89410e9009e4474b9212ed0370a')

    // it's to Client A
    assert.equal(bytesToHex(packet.destination_hash), '072ec44973a8dee8e28d230fb4af8fe4')
    const recipient = recipients[bytesToHex(packet.destination_hash)]
    assert.ok(recipient)

    // Decrypt the packet data first
    const decrypted = message_decrypt(packet, recipient.pub, ratchets)
    assert.ok(decrypted, 'message should decrypt')

    // Parse the LXMF message
    const message = lxmf_parse(decrypted, packet.destination_hash, pubkeys['76a93cda889a8c0a88451e02d53fd8b9'])
    assert.ok(message)

    assert.equal(bytesToHex(message.source_hash), '76a93cda889a8c0a88451e02d53fd8b9')
    assert.equal(bytesToHex(message.signature), '164bd6078866c67d5997ec8871d60125b24b4778be092c7ba6b5a20c3dad7a1c98a0382d4d77771edf93c96d78a668a962804a8009220d5ff3e8e9912718c809')
    assert.equal(message.timestamp, 1759383650.12609)

    // Decode title and content if they are Uint8Array
    const title = message.title instanceof Uint8Array ? new TextDecoder().decode(message.title) : message.title
    const content = message.content instanceof Uint8Array ? new TextDecoder().decode(message.content) : message.content

    assert.equal(title, '')
    assert.equal(content, 'hello from B')
    // Convert Map to object for comparison
    const fields = message.fields instanceof Map ? Object.fromEntries(message.fields) : message.fields
    assert.deepEqual(fields, {})

    // Verify signature
    const sender = recipients[bytesToHex(message.source_hash)]
    assert.ok(sender, 'valid sender')
    assert.ok(message.valid, 'valid message signature')

    // save for PROOF validation
    sentPackets[bytesToHex(packet.packet_hash.slice(0, 16))] = {
      recipientHash: packet.destination_hash,
      packet_hash: packet.packet_hash,
      raw: packets[4]
    }
  })
})

describe('PROOF', () => {
  test('2831d76f1a8035638505c132fe5818c1 (A -> B)', () => {
    const packet = packet_unpack(packets[3])
    assert.equal(packet.packet_type, PACKET_PROOF)
    assert.equal(bytesToHex(packet.destination_hash), '2831d76f1a8035638505c132fe5818c1')
    assert.equal(bytesToHex(packet.packet_hash), 'c6c8d3a2da7de271b3262ed73f8d07f2d9b665e6dd382c610b2761f3484a6979')

    // lookup DATA packet that we "sent" for recipient & full-hash (only we know that, because we sent it)
    const sent = sentPackets[bytesToHex(packet.destination_hash)]
    assert.ok(sent)
    assert.equal(bytesToHex(sent.packet_hash), '2831d76f1a8035638505c132fe5818c1d1d25869a973d35c197d669f0d5074d8')

    // it's from Client B
    assert.equal(bytesToHex(sent.recipientHash), '76a93cda889a8c0a88451e02d53fd8b9')
    const recipient = recipients[bytesToHex(sent.recipientHash)]
    assert.ok(recipient)

    const valid = validate_proof(packet, recipient.pub, sent.packet_hash)
    assert.ok(valid)
  })

  test('d7c0e833f0cbde9f9133cd9e7d508b1a (B -> A)', () => {
    const packet = packet_unpack(packets[5])
    assert.equal(packet.packet_type, PACKET_PROOF)
    assert.equal(bytesToHex(packet.destination_hash), 'd7c0e833f0cbde9f9133cd9e7d508b1a')
    assert.equal(bytesToHex(packet.packet_hash), '3e98d0daf2b23edece8737b0ca348a04d882b1a4800b375259a6b03a1fa3b428')

    // lookup DATA packet that we "sent" for recipient & full-hash (only we know that, because we sent it)
    const sent = sentPackets[bytesToHex(packet.destination_hash)]
    assert.ok(sent)
    assert.equal(bytesToHex(sent.packet_hash), 'd7c0e833f0cbde9f9133cd9e7d508b1a61d2c89410e9009e4474b9212ed0370a')

    // it's from Client A
    assert.equal(bytesToHex(sent.recipientHash), '072ec44973a8dee8e28d230fb4af8fe4')
    const recipient = recipients[bytesToHex(sent.recipientHash)]
    assert.ok(recipient)

    const valid = validate_proof(packet, recipient.pub, sent.packet_hash)
    assert.ok(valid)
  })
})

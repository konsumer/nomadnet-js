...
describe('NodeJS Encryption Helpers', () => {
  describe('Identity Key Generation', () => {
    test('private_identity generates 64 bytes', async ({ assert }) => {
      const priv_key = await private_identity()
      assert.ok(priv_key instanceof Uint8Array)
      assert.equal(priv_key.length, 64) // 32 bytes X25519 + 32 bytes Ed25519
    })

    test('public_identity derives 64 byte public key', async ({ assert }) => {
      const priv_key = await private_identity()
      const pub_key = await public_identity(priv_key)
      assert.ok(pub_key instanceof Uint8Array)
      assert.equal(pub_key.length, 64) // 32 bytes X25519 pub + 32 bytes Ed25519 pub
    })

    test('public_identity contains both X25519 and Ed25519 keys', async ({ assert }) => {
      const priv_key = await private_identity()
      ...
      const expected_x25519_pub = await x25519_public_for_private(x25519_priv)
      assert.deepEqual(x25519_pub, expected_x25519_pub)

      // Verify Ed25519 public key derivation
      const expected_ed25519_pub = await ed25519_public_for_private(ed25519_priv)
      assert.deepEqual(ed25519_pub, expected_ed25519_pub)
    })

    test('public_identity throws with invalid input length', async ({ assert }) => {
      await assert.rejects(async () => {
        await public_identity(new Uint8Array(10))
      })

      await assert.rejects(async () => {
        await public_identity(new Uint8Array(100))
      })
    })
  })
  ...
  describe('Ratchet Key Generation', () => {
    ...
    test('public_ratchet derives 32 byte public key', async ({ assert }) => {
      const priv_key = private_ratchet()
      const pub_key = await public_ratchet(priv_key)
      assert.ok(pub_key instanceof Uint8Array)
      assert.equal(pub_key.length, 32)
    })

    test('public_ratchet throws with invalid input', async ({ assert }) => {
      await assert.rejects(async () => {
        await public_ratchet(new Uint8Array(10))
      })
    })
  })

  describe('Hash Functions', () => {
    test('sha256 produces 32 byte hash', async ({ assert }) => {
      ...
      const different_result = await sha256(different_data)
      assert.notDeepEqual(result, different_result)
    })

    test('hmac_sha256 produces 32 byte HMAC', async ({ assert }) => {
      const key = new TextEncoder().encode('secret_key')
      const data = new TextEncoder().encode('message')
      const result = await hmac_sha256(key, data)
      assert.ok(result instanceof Uint8Array)
      assert.equal(result.length, 32)

      // Different key produces different result
      const different_key = new TextEncoder().encode('different_key')
      const different_result = await hmac_sha256(different_key, data)
      assert.notDeepEqual(result, different_result)

      // Different data produces different result
      const different_data = new TextEncoder().encode('different_message')
      const different_result2 = await hmac_sha256(key, different_data)
      assert.notDeepEqual(result, different_result2)
    })
  })

  describe('HKDF', () => {
    test('hkdf derives key of specified length', async ({ assert }) => {
      const ikm = new TextEncoder().encode('input_key_material')
      const length = 32
      const result = await hkdf(ikm, length)
      assert.ok(result instanceof Uint8Array)
      assert.equal(result.length, length)
    })

    test('hkdf with salt and info', async ({ assert }) => {
      const ikm = new TextEncoder().encode('input_key_material')
      const salt = new TextEncoder().encode('salt_value')
      const info = new TextEncoder().encode('info_value')
      const length = 64
      const result = await hkdf(ikm, length, salt, info)
      assert.ok(result instanceof Uint8Array)
      assert.equal(result.length, length)
    })

    test('hkdf with different params produces different output', async ({ assert }) => {
      const ikm1 = new TextEncoder().encode('input1')
      const ikm2 = new TextEncoder().encode('input2')
      const length = 32
      const result1 = await hkdf(ikm1, length)
      const result2 = await hkdf(ikm2, length)
      assert.notDeepEqual(result1, result2)
    })

    test('hkdf throws with empty ikm', async ({ assert }) => {
      await assert.rejects(async () => {
  ...
  describe('AES', () => {
    test('aes_cbc_encrypt and decrypt', async ({ assert }) => {
      ...
      const ciphertext = await aes_cbc_encrypt(key, iv, plaintext)
      const decrypted = await aes_cbc_decrypt(key, iv, ciphertext)
      assert.deepEqual(decrypted, plaintext)
    })

    test('different IV produces different ciphertext', async ({ assert }) => {
      ...
      const ciphertext1 = await aes_cbc_encrypt(key, iv1, plaintext)
      const ciphertext2 = await aes_cbc_encrypt(key, iv2, plaintext)

      assert.notDeepEqual(ciphertext1, ciphertext2)
    })

    test('wrong key throws or produces different data', async ({ assert }) => {
      const key1 = crypto.getRandomValues(new Uint8Array(32))
      const key2 = crypto.getRandomValues(new Uint8Array(32))
      const iv = crypto.getRandomValues(new Uint8Array(16))
      const plaintext = new TextEncoder().encode('this is a longer test message')

      const ciphertext = await aes_cbc_encrypt(key1, iv, plaintext)

      // Web Crypto API may throw an error when decrypting with wrong key
      // This is actually correct behavior
      try {
        const decrypted = await aes_cbc_decrypt(key2, iv, ciphertext)
  ...
  describe('Ed25519', () => {
    test('ed25519_sign and verify', async ({ assert }) => {
      const private_key = crypto.getRandomValues(new Uint8Array(32))
      const public_key = await ed25519_public_for_private(private_key)
      const message = new TextEncoder().encode('test message for signing')

      const signature = await ed25519_sign(private_key, message)
      const is_valid = await ed25519_validate(signature, message, public_key)

      assert.equal(is_valid, true)
    })

    test('invalid signature fails verification', async ({ assert }) => {
      const private_key1 = crypto.getRandomValues(new Uint8Array(32))
      const private_key2 = crypto.getRandomValues(new Uint8Array(32))

      const public_key1 = await ed25519_public_for_private(private_key1)
      const public_key2 = await ed25519_public_for_private(private_key2)

      const message = new TextEncoder().encode('test message')
      const signature = await ed25519_sign(private_key1, message)
      const is_valid = await ed25519_validate(signature, message, public_key2)

      assert.equal(is_valid, false)
    })

    test('different message fails verification', async ({ assert }) => {
      const private_key = crypto.getRandomValues(new Uint8Array(32))
      const public_key = await ed25519_public_for_private(private_key)
      ...
      const signature = await ed25519_sign(private_key, message1)
      const is_valid = await ed25519_validate(signature, message2, public_key)

      assert.equal(is_valid, false)
    })

    test('ed25519_public_for_private derives public key', async ({ assert }) => {
      const private_key = crypto.getRandomValues(new Uint8Array(32))
      const public_key = await ed25519_public_for_private(private_key)
    ...
    test('ed25519 from identity key', async ({ assert }) => {
      const identity_priv = await private_identity() // 64 bytes: X25519(32) + Ed25519(32)
      const ed25519_priv = identity_priv.slice(32, 64) // Extract Ed25519 private key
      const ed25519_pub = await ed25519_public_for_private(ed25519_priv)

      // Test signing with the Ed25519 portion of the identity key
      const message = new TextEncoder().encode('signed with identity key')
      const signature = await ed25519_sign(ed25519_priv, message)
      const is_valid = await ed25519_validate(signature, message, ed25519_pub)
  ...
  describe('X25519', () => {
    test('x25519_key_exchange produces shared secret', async ({ assert }) => {
      const alice_private = private_ratchet()
      const bob_private = private_ratchet()

      const alice_public = await x25519_public_for_private(alice_private)
      const bob_public = await x25519_public_for_private(bob_private)

      // Alice computes shared secret using her private key and Bob's public key
      const shared_secret_alice = await x25519_exchange(alice_private, bob_public)

      // Bob computes shared secret using his private key and Alice's public key
      const shared_secret_bob = await x25519_exchange(bob_private, alice_public)
    ...
    test('x25519_public_for_private derives public key', async ({ assert }) => {
      const private_key = private_ratchet()
      const public_key = await x25519_public_for_private(private_key)

      assert.ok(public_key instanceof Uint8Array)
      assert.equal(public_key.length, 32)
    })

    test('x25519 from identity key', async ({ assert }) => {
      const identity_priv = await private_identity() // 64 bytes: X25519(32) + Ed25519(32)
      const x25519_priv = identity_priv.slice(0, 32) // Extract X25519 private key
      const x25519_pub = await x25519_public_for_private(x25519_priv)

      // Test key exchange with the X25519 portion of the identity key
      const other_priv = private_ratchet()
      const other_pub = await x25519_public_for_private(other_priv)

      const shared_secret_1 = await x25519_exchange(x25519_priv, other_pub)
      const shared_secret_2 = await x25519_exchange(other_priv, x25519_pub)
  ...
  describe('Edge Cases', () => {
    test('hash functions with empty inputs', async ({ assert }) => {
      // Hash functions with empty data
      const empty_hash = await sha256(new Uint8Array(0))
      assert.ok(empty_hash instanceof Uint8Array)
      assert.equal(empty_hash.length, 32)

      // HMAC with empty data
      const empty_hmac = await hmac_sha256(new TextEncoder().encode('key'), new Uint8Array(0))
  ...
  describe('Identity Key Structure', () => {
    test('identity key contains both X25519 and Ed25519 keys', async ({ assert }) => {
      const identity_priv = await private_identity()
      ...
      const x25519_pub = await x25519_public_for_private(x25519_part)
      const ed25519_pub = await ed25519_public_for_private(ed25519_part)

      assert.equal(x25519_pub.length, 32)
      assert.equal(ed25519_pub.length, 32)
    })

    test('identity key derivation is consistent', async ({ assert }) => {
      const identity_priv = await private_identity()
      const pub_identity_1 = await public_identity(identity_priv)
      const pub_identity_2 = await public_identity(identity_priv)
